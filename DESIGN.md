...
- **Plan block** (`.plan-block`): panel bg-2, titlebar `terraform plan · career/manny-flores`, mono 13px lines with full-width tinted line backgrounds, `overflow-x: auto`, staggered per-line reveal on view (reduced-motion collapses to instant). Exposed to assistive tech as `role="img"` with a prose `aria-label` summarizing the career facts; the styled lines are `aria-hidden`. The engineer lede states the legend (`+` built, `~` changed, `-` removed). A `# comment` after a run of spaces is its own span (`.plan-comment`); on phones `white-space: normal` collapses the alignment padding and the comment drops to a second line, so nothing scrolls sideways on touch. In the recruiter lens it becomes `.plan-block--annotated`: titlebar gains a clay `· annotated` flag, lines wrap with a hanging indent, and each carries a sans `.plan-gloss` beneath it (72ch measure). Every line has a gloss; a line without one does not ship.
...
- **Contact** (`.section--contact`): the page ends on the ask, not on a chip cloud. Headline, one lede line per lens, email as the primary button, LinkedIn and Resume ghost.
...
- **Do** write both sides of every `{ eng, rec }` pair when adding content. The recruiter side is plain English (ownership, scale, outcome, no protocol acronyms); it never adds a claim the engineer side lacks (tests enforce presence, the author enforces parity).
...
- **Don't** add a third role. Engineer and recruiter cover the audience; more lenses multiply copy without adding clarity.
