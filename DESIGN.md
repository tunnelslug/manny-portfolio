...
- **Role lens** (`.role-switch`, `RoleSwitch.jsx`, `src/lib/role.js`): `read as  [engineer] [recruiter]`. Two audiences, two questions: engineer asks how it is built, recruiter asks what was owned, at what scale, with what outcome. The recruiter lens is not a simplification and never dumbs down; it keeps system names and speaks in ownership, scale, and results. Every lensed string lives in `src/content/portfolio.js` as an `{ eng, rec }` pair, rendered through `<Lens>` (a keyed span whose remount replays the 360ms `.lens` fade). Persisted in `localStorage` (`mf-role`) through `useSyncExternalStore`, so prerendered HTML hydrates as engineer and re-renders into the stored role without a mismatch. The plan lede also carries an inline switch ("Hiring? Read it as a recruiter.") so the lens is discoverable mid-page. The boot ceremony's session card reads the stored role.
...
- **Do** write both sides of every `{ eng, rec }` pair when adding content. The recruiter side reframes (ownership, scale, outcome); it never adds a claim the engineer side lacks (tests enforce presence, the author enforces parity).
