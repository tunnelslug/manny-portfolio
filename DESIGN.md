...
- **Access fabric** (`.ig`, `IdentityGraph.jsx`): the hero figure. Portrait = Manny at the center of a 420x440 SVG; seven system nodes hang off it on 1px `--color-border` edges with mono labels (name 11px/500, sub 9.5px). Node data (`fabricNodes`) is limited to systems that already appear in scope rows or stack chips. Pulses are SMIL `animateMotion` circles: clay outward for systems he configures (Okta, GCP, Workspace, SaaS), muted inward for identities arriving into the fabric (service accounts, AI agents, and M&A). The M&A node stands for acquisition integration as a capability: acquired companies' apps and identity providers brought onto the Okta standard. Standalone IdPs are retired afterward (Entra ID is named as the largest of those); duplicates of systems already in the fabric, such as a second Google Workspace, are merged rather than retired. The rest stay generic on purpose (plural first, one example, no roster to verify or to be pigeonholed by). `retired: true` on a node still renders the dashed treatment if a truly decommissioned system ever needs to appear. Hover or focus a node for a one-line brief in the readout under the graph (all briefs share one grid cell so the block never changes height; idle shows a short hint). On phones there is no readout: the brief opens in a fixed sheet at the bottom edge (`.ig-sheet`), closed by its button, a tap elsewhere, or scrolling, so the hero never moves; click pins it (`aria-pressed`) and logs `graph.inspected`. Draw-in (edges trace via `stroke-dasharray: var(--len)`, nodes fade) runs when `live` flips, which App sets from the boot ceremony's `onDone`, so it is never hidden behind the boot overlay. `useReducedMotion` renders no pulses and no draw-in. On phones (`max-width: 768px`, via `useMediaQuery`) the same node data renders in the `compact` layout: a 360x196 viewBox with the portrait on the left and all seven nodes fanned into one column on the right, name-only labels, row-shaped hit areas. The hero restacks fabric, CTAs, then name and lede, so the graph and all four actions share the first screen; a `MobileDock` bar then carries the same four actions at the bottom edge once the hero CTAs scroll away (hidden while the resume dialog or menu is open). Prerendered HTML is the full layout; the compact box is sized in CSS so the swap causes no layout shift.
